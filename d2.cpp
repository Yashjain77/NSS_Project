...
void solve(){
    ...
    cin>>s;
    s="!"+s;
    vector <vector <vector <vector <vll>>>> dp(n+2,vector <vector <vector <vll>>> (n+2,vector <vector <vll>> (n+2,vector <vll> (2,vll (5,0)))));
    if(s[1]=='('){
        for(int i=0;i<5;i++){
            dp[1][1][1][0][i]=1;
        }
    ...
    for(int i=2;i<=n;i++){
        for(int j=0;j<=i;j++){
            // dp[i][0]=1;
                for(int size=j;size<=i;size++){
                    if(s[i]=='('){
                        dp[i][j][size][1][0]=dp[i-1][j][size][1][0];
                        if(j!=0)
                        dp[i][j][size][1][1]=(dp[i-1][j-1][size-1][1][0]+dp[i-1][j][size][1][1])%MOD;
                        else
                        dp[i][j][size][1][1]=dp[i-1][j][size][1][1];
                        if(j!=0)
                        dp[i][j][size][1][2]=(dp[i-1][j-1][size-1][1][2]+dp[i-1][j-1][size-1][1][1]+dp[i-1][j][size][1][2])%MOD;
                        else
                        dp[i][j][size][1][2]=dp[i-1][j][size][1][2];
                        if(j!=0)
                        dp[i][j][size][0][0]=(dp[i-1][j][size][0][0]+dp[i-1][j-1][size-1][0][0])%MOD;
                        else
                        dp[i][j][size][0][0]=dp[i-1][j][size][0][0];
                    }
                    else{
                        if(size!=0)
                        dp[i][j][size][1][0]=(dp[i-1][j+1][size-1][0][0]+dp[i-1][j][size][1][0]+dp[i-1][j+1][size-1][1][0])%MOD;
                        else
                        dp[i][j][size][1][0]=dp[i-1][j][size][1][0];
                        for(int k=1;k<5;k++){
                            if(size!=0)
                            dp[i][j][size][1][k]=(dp[i-1][j+1][size-1][1][k]+dp[i-1][j][size][1][k])%MOD;
                            else
                            dp[i][j][size][1][k]=dp[i-1][j][size][1][k];
                        }
                        dp[i][j][size][0][0]=dp[i-1][j][size][0][0];
                    }
                }
    ...
    for(int size=2;size<=n;size++){
        ans=(ans+((dp[n][0][size][1][2])%MOD)*(size-2)%MOD)%MOD;
    }
    cout<<ans<<endl;
}
